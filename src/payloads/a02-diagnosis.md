...
- 담지 않는다: 최종 제안서 문장, 화면 설계, 견적 로직. 이건 이 문서에서 **파생**되는 것이지 여기 저장하는 게 아니다.

## 1. 최소 입력 = 페인 1건

나머지는 전부 선택이다. 못 채워서 시작을 못 하는 것보다, 얕게라도 시작해 미팅에서 채우는 게 낫다.
빈 곳은 삭제하지 말고 **가정 배지**를 붙여 남긴다 — 무엇을 모르는지가 다음 미팅의 질문지가 된다.
...
12. **미기재는 갭이 아니라 질의 대상이다.** 고객이 말하지 않은 것을 없는 것으로 처리하지 않는다.
...
- **갭은 요구 관계지 대상 비교가 아니다.** "같은 대상의 현재 vs 목표"로 맞추려 하면 자료 명사와 업무 명사가 층위가 달라 짝이 안 맞는다. "산출물이 요구하는 것 ↔ 현재가 제공하는 것"으로 대조한다.
- **`current`가 아예 없으면 갭이 아니라 가정 배지다.** 있는데 요구에 못 미칠 때만 갭이다. 미확인이 확인되는 순간 배지가 갭으로 승격한다.
- 도출은 자동, **표출은 승인 후.**
...
이 상태로도 진단은 성립한다. `actors`가 비어 가정 배지가 붙어 있고, `verify.question`이 다음 미팅의 질문지다.
