...
**계약은 데모를 만들기 전에, 늦어도 데모 v0.1 직후에 쓴다.** 다 만들고 쓰면 계약이 아니라 회고다.
...
**②와 ③이 이 절의 존재 이유다.** 같은 값을 여러 곳에서 읽는 구조는
데모에서는 안 보이다가 정본에서 터진다. **몇 곳인지 세어서 숫자로 적는다.**
...
**현황 줄에 "정본 미편집"을 명시한다.** 데모 세션이 정본을 건드리면 계약이 아니라 충돌이다.

## 금칙

- 데모를 다 만든 뒤에 계약 쓰기 — 그건 회고다
