...
> 이 프로젝트의 문서가 **몇 개이고, 무엇이 무엇에 의존하며, 어떤 순서로 써야 하는지**의 단일 지도.
> 새 문서를 만들면 **같은 커밋에서** 이 표를 갱신한다.

## 0. 이 프로젝트가 만드는 것 (한 줄)

{한 문장. 이 줄이 안 써지면 문서를 더 쓸 때가 아니라 범위를 정할 때다.}
...
층마다 표 하나. 각 행에 **상태**를 반드시 단다 — 상태 없는 문서는 읽는 사람이 믿을지 말지 모른다.
...
**이 절이 이 문서의 존재 이유다.** 어디까지 왔는지의 기준점이 여기다.
...
- 이 문서가 길어지면 문서가 너무 많은 것이다. 지도가 아니라 영토를 줄인다.
