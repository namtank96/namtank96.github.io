...
- **Display**: Noto Serif KR, weight **500/600**, style normal.
...
### 웨이트 — 한글 명조는 크기로 위계를 만듭니다

**700을 로드하지 않습니다.** 제목마다 700을 쓰면 편집물이 아니라 보고서 인상이 됩니다.
웨이트는 두 단만 쓰고 나머지는 크기가 담당합니다.

| 자리 | 크기 | 웨이트 |
|---|---|---|
| 워드마크 · 홈 표제 · 페이지 제목 | 32–52px | **600** |
| 섹션 제목 | 24px | **600** |
| 항목 제목(카드·행·판단·회사명) | 17–19px | **500** |

같은 위계에 다른 값을 쓰지 않습니다. 한글 명조 17~19px 에서 600과 700은 눈으로 구분되지 않습니다 —
값만 흩어지고 위계는 안 생깁니다.

### 자간 — 음수 금지

**Display tracking: 0.** 음수 자간은 라틴 디스플레이 조판의 관습입니다. 한글 음절은 이미
정사각 틀에 균등 배치돼 있어서 음수를 주면 ㅁ·ㅇ·ㅎ 의 속공간이 닫히고 글자가 번져 보입니다.
소형 라벨의 양수 자간도 **0.04em 을 넘기지 않습니다**. 0.08~0.1em 은 라틴 소형 대문자의 값이고,
한글에 주면 어절이 풀어져 라벨로 안 읽힙니다.

### 표제 줄바꿈 — 손으로 끊습니다

**`text-wrap: balance` 를 쓰지 않습니다.** `keep-all` 때문에 줄바꿈 기회가 어절 경계뿐인데,
balance 는 어절만 보고 의미 단위를 보지 않습니다. "확정된 게 / 없는" 처럼 한 덩어리를 갈라 놓습니다.

표제는 `src/lib/profile.ts` 에서 마커로 직접 지정합니다.

- `|` 항상 줄바꿈
- `/` 좁은 화면(< 40rem)에서만 줄바꿈

`headlineLines()` 가 `줄[조각[]]` 로 돌려주고, `.line` 은 block, `.seg` 는 좁은 화면에서만 block 입니다.
표제에 `max-width: Nch` 를 함께 걸지 마세요 — 손으로 정한 줄바꿈이 무너집니다.
콘텐츠에서 오는 제목(`/work/[id]`, `/assets/[id]`)은 마커 없이 자연 줄바꿈에 맡깁니다.
